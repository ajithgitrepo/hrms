...
def get_custom_leave_type_bychange(request):
    ...
    cursor = connection.cursor()
    cursor.execute("SELECT lbl.balance ,lbl.type as customtype, lt.name , lt.code , lt.unit , lt.type, lbl.leave_type_id  FROM leave_balance lbl LEFT JOIN leave_type lt ON lbl.leave_type_id = lt.id WHERE lt.is_active='1' and lbl.is_active= '1' and lbl.employee_id = '"+emp_id+"'   ")
    objs = cursor.fetchall()
    json_data = []
