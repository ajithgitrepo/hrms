...
def exit_details(request):
   # return HttpResponse("employee")
    employee = Exit_Employee_Detail.objects.filter(is_active__in=[0, 1])
   # return HttpResponse(employee)
    print(employee)
...
def add_exit_details(request):  
   ...
   # """
    ...
    data = Employee.objects.get(employee_id=employee)
    data.is_active = 0
    data.save()

    user = User.objects.get(emp_id=employee)
    user.is_active = 0
    user.save()
    #
...
def status_exit_employees(request, pk, val):
      #return HttpResponse(pk)
      data = Exit_Employee_Detail.objects.get(employee_id=pk)
      data.is_active = val
      data.save()
      
      employee_name = Employee.objects.filter(employee_id=pk).update(is_active= val)
      user = User.objects.filter(emp_id=pk).update(is_active= val)

      messages.info(request, 'Employee status was changed! ')
      return redirect('exit_details')