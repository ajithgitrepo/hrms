...
def get_custom_leave_type(request):
    ...
    result = Leave_Effective.objects.filter(
        is_active='1')  # , leave_type_id = leave_id)
    # else:
...
def add_custom_leave_type(request):
   ...
   # effective_period = request.POST.get('effective_period')
    ...
    if not Customize_Leave_Effective.objects.filter(Q(employee_id=employee_id) and Q(leave_type_id=leave_type_id)).exists():
        ...
        effective_id = Customize_Leave_Effective.objects.only('id').get(
            employee_id=employee_id, leave_type_id=leave_type_id).id
 ...
 #   queryset = Leave_Type.objects.filter(id__in = result.values('leave_type_id'))
  ...
  #  jsondata = serializers.serialize('json', data)
    ...
    json_data = []
    for obj in objs:
        json_data.append({"balance": obj[0], "customtype": obj[1], "name": obj[2],
                         "code": obj[3], "unit": obj[4], "type": obj[5], "leave_type_id": obj[6]})
