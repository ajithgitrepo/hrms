...
def leave_request(request):
    ...
    if(role == "Admin"):
        leave_request = LeaveRequest.objects.filter(
            is_active='1', employee__is_active=1).order_by('created_at').reverse()
        # print(leave_request)
    if(role == "Manager"):
        reporting_ids = Reporting.objects.filter(
            is_active='1', reporting_id=request.user.emp_id)
        ...
        leave_request = LeaveRequest.objects.filter(
            is_active='1', employee_id__in=ids, employee__is_active=1).order_by('created_at').reverse()
        # print(leave_request)
        ...
        if form.is_valid():
            ...
            if request.FILES:
                ...
                if upload != None:
                    logoRoot = os.path.join(
                        settings.MEDIA_ROOT, 'leave_documents/')
           ...
           # return HttpResponse(employee_id)
            ...
            if todate != None and fromdate != None:
                ...
                if (str(total_days) < str(minimum_leave_apply)) and (minimum_leave_apply != None) and (message == ""):
                    # if total_days != 0:
                    message = "Minimum " + minimum_leave_apply + \
                        " leave that can be availed per application"
                    # return HttpResponse(message)
                if (message == "") and (maximum_leave_apply != None):
                    if int(maximum_leave_apply) < int(total_days):
                        message = "Maximum " + maximum_leave_apply + \
                            " leave that can be availed per application"
                if (message == "") and (maximum_consecutive_leave_apply != None):
                    if(int(total_days) > int(maximum_consecutive_leave_apply)):
                        message = "Maximum " + maximum_consecutive_leave_apply + \
                            "  number of consecutive days of Leave allowed"
                if minimum_gap_apply != None:
                    applic = "ok"
                    # return HttpResponse(applic)

                    last_applied = LeaveRequest.objects.filter(
                        employee_id=employee_id).order_by('from_date').reverse()
                    ...
                    if last_applied != None and len(last_applied) < 0:
                        ...
                        if(int(minimum_gap_apply) >= int(total_days1)):
                            applic = "ok"
                            message = "Require minimum " + minimum_gap_apply + \
          ...
          #  return HttpResponse(message)
            ...
            g1 = LeaveRequest.objects.create(
                ...
                leave_type=Leave_Type.objects.get(
                    id=leave_type) if leave_type else None,
                from_date=fromdate,
                to_date=todate,
                ...
                added_by=Employee.objects.get(
                    employee_id=request.user.emp_id) if request.user.emp_id else None,
                is_active=is_active,
                device=device,
            ...
            for i in range(delta.days + 1):
                start_date = datetime.strptime(
                    request.POST.get('from_date'), '%d-%m-%Y')
                ...
                if not Attendance.objects.filter(Q(employee_id=emp_id, date=datetime.strftime(day, "%Y-%m-%d"))).exists():
                    insert = Attendance.objects.create(date=datetime.strftime(
                        day, "%Y-%m-%d"), employee_id=emp_id, is_leave=1, is_half=is_half)
                else:
            ...
            employee_id = request.POST.get('employee')
            emp_dtl = list(Employee.objects.values_list(
                'first_name', 'last_name', 'employee_id', flat=False).get(employee_id=employee_id))
            emp_full = emp_dtl[0] + emp_dtl[1] + " ( " + emp_dtl[2] + " )"

            user_employee_id = request.POST.get('employee')
            report_id = list(Reporting.objects.values_list(
                'reporting_id', flat=False).get(employee_id=user_employee_id))

            report_emp_dtl = list(Employee.objects.values_list(
                'first_name', 'last_name', 'employee_id', 'email_id', flat=False).get(employee_id=report_id[0]))
            # return HttpResponse((name[1]))
            reason1 = request.POST.get('reason')
            # return HttpResponse(reason)
            subject = 'Leave Request'
            from_date1 = datetime.strptime(
                request.POST.get('from_date'), '%d-%m-%Y').date()
            to_date1 = datetime.strptime(
                request.POST.get('to_date'), '%d-%m-%Y').date()
            html_message = render_to_string('mail_templates/leave_request_format.html', {
                                            'fname': report_emp_dtl[0], 'lname': report_emp_dtl[1], 'lv_type': leave_name, 'f_date': from_date1, 't_date': to_date1, 'reason': reason1, 'emp_name': emp_full})
...
def change_leave_status(request):
    ...
    type_mode = LeaveRequest.objects.values_list(
        'leave_mode', flat=True).get(id=id)
    ...
    if update:
        ...
        if (value != '0'):
            ...
            lv_dtl = list(LeaveRequest.objects.values_list(
                'employee_id', 'from_date', 'to_date', 'reason', flat=False).get(id=id))

            employee_id = lv_dtl[0]
            emp_dtl = list(Employee.objects.values_list('first_name', 'last_name',
                           'employee_id', 'email_id', flat=False).get(employee_id=employee_id))

            user_employee_id = request.user.emp_id
            report_emp_dtl = list(Employee.objects.values_list(
                'first_name', 'last_name', 'employee_id', 'email_id', flat=False).get(employee_id=user_employee_id))
            ...
            html_message = render_to_string('mail_templates/leave_approval_format.html', {
                                            'fname': emp_dtl[0], 'lname': emp_dtl[1], 'f_date': from_date_new, 't_date': to_date_new, 'reason': reason1, 'status': status, 'emp_name': emp_full1})
            plain_message = strip_tags(html_message)
            email_from = settings.EMAIL_HOST_USER
