...
def compensatory_request_details(request):
    ...
    if role == 'Admin':
        employee = Compoensatory_Request_Detail.objects.filter(
            is_active='1').order_by('-created_at')
        # print(employee)
    ...
    if role == 'Manager':
        ...
        employee = Compoensatory_Request_Detail.objects.filter(
            is_active='1', employee_id__in=ids, employee__is_active=1).order_by('-created_at')
        # print(employee)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
           ...
           # customer_name = request.POST.get('customer_name')
            ...
            obj.save()
            messages.success(
                request, 'Compensatory request details was added ! ')
            return redirect('compensatory_request_details')

    role = role_name(request)

    if role == 'Admin':
        employee = Employee.objects.filter(is_active=1).exclude(
...
def compensatory_request_status(request):
    ...
    employee = Leave_Balance.objects.get(
        is_active=1, employee_id=data.employee_id, leave_type_id=3)
        ...
        if value == '1':
            ...
            if data.unit == "Hours":
                ...
                duration = datetime.combine(
                    date.today(), to_time)-datetime.combine(date.today(), from_time)
